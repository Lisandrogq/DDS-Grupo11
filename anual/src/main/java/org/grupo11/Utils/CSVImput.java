...
public class CSVImput {
    ...
    public CSVImput(DocumentType documentType, int document, String name, String surname, String mail,
            long contributionDate, ContributionTypeField contributionType, int quantity) {
    ...
    public static CSVImput processField(String field) throws ParseException {
        try {
            ...
            if (!Arrays.stream(ContributionTypeField.values())
                    .anyMatch((ContributionTypeField c) -> c.name().equals(fields.get(6)))) {
            ...
            return new CSVImput(documentType, document, name, surname, mail, contributionDate, contributionType,
                    quantity);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
